...
def gbuffers():
    ...
    set_auxiliary_cvar(
        "r_AntialiasingMode", int(general.get_cvar("r_AntialiasingMode")), 0
    )
...
def profiling():
    ...
    rStats = cvar(
        "r_stats", 0, "profiling", "R_STATS - Review CPU \ GPU \ Frame \ Asset Timings."
    )
...
def ai():
    ...
    aiDebugDraw = cvar(
        "ai_DebugDraw", -1, "ai", "ai_DebugDraw - AI and Nav Mesh Debug Views."
    )
    ...
    try:
        ...
        # AI is a special case where we have to execute_external_icon_exe a command before we can see the information we want.
        # But there is no guarentee a game will support that agent type, so catch the exception
    try:
        general.execute_command(
            "ai_debugMNMAgentType MediumSizedCharacters"
        )  # MediumActor # Saving for reference for later
        general.execute_command("ai_debugMNMAgentType VehicleMedium")  # LargeActor
    except RuntimeError as exc:
        print(
            "MediumSizedCharacters, and/or VehicleMedium are invalid agent types for your game."
        )

        # AI debug view will not display anything unless there are active AI and MNM built. Warn user about this.
    print(
        "NOTE: This view is only useful if there are active AI on screen, and Multi-layer Navigation Mesh generated."
    )
...
def materialfx():
    ...
    mfxDebug = cvar(
        "mfx_Debug", 0, "materialfx", "Show material collisions to verify impact type."
    )
    mfxDebug.add_value(1, "Material Impact Debug - 1.")
    mfxDebug.add_value(2, "Material Impact Debug - 2.")
    mfxDebug.add_value(
        3, "Material Impact Debug - Draw collisions and their surface type."
    )
...
if __name__ == "__main__":
    # GLOBAL NOTE:
    # - All python scripts should execute_external_icon_exe through a main() function.
